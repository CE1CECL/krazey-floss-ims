...
object SipUplinkMediaRtpSender {
    fun sendStorageFrame(
        ...
        val packet =
            SipAmrRtpPayload.buildBandwidthEfficientRtpPacketFromStorageFrame(
        ...
        try {
            ...
            if (realFrameCount < 10) {
                Rlog.d(
                    logTag,
                    "Sent RTP packet #$sequenceNumber ft=$frameType ts=$timestamp payload=${packet.drop(
                        12,
                    ).take(4).joinToString(" ") { "%02x".format(it) }}... to $remoteAddr:$remotePort",
                )
            }
            if (realFrameCount == 0) {
