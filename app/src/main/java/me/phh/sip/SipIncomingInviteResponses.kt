        ...
        val rewritten =
            lines
                .map { line ->
                    ...
                    val withConf =
                        if (mapped.any { it.startsWith("a=conf:qos remote", ignoreCase = true) }) {
                            mapped
                        } else {
...
internal object SipIncomingInviteFinalResponses {
    ...
    fun nextFinalResponseRetransmitDelayMs(currentDelayMs: Long): Long = (currentDelayMs * 2).coerceAtMost(4_000L)

    fun acceptWithoutValidIncomingCallLog(call: Any?): String = "acceptCall without valid incoming currentCall: $call"
    ...
    fun finalResponseMissingAckCancellationExtras(acceptedCallId: String): Map<String, String> = mapOf("call-id" to acceptedCallId)

    fun finalResponseRetransmitLog(
        acceptedCallId: String,
        elapsedMs: Long,
    ): String = "Retransmitting incoming 200 OK waiting for ACK callId=$acceptedCallId elapsed=${elapsedMs}ms"

    fun finalResponseRetransmitWriteContext(
        acceptedCallId: String,
        elapsedMs: Long,
    ): String = "incoming INVITE final 200 OK retransmit callId=$acceptedCallId elapsed=${elapsedMs}ms"

    fun finalResponseRetransmitWriteFailureLog(
        acceptedCallId: String,
        elapsedMs: Long,
    ): String = "Stopping incoming 200 OK retransmit after write failure callId=$acceptedCallId elapsed=${elapsedMs}ms"

    fun finalResponseMissingAckTimeoutLog(
        acceptedCallId: String,
        elapsedMs: Long,
    ): String = "Incoming accepted call still has no ACK after ${elapsedMs}ms; clearing pending accepted state callId=$acceptedCallId"
    ...
    fun duplicateFinalResponseWriteContext(
        incomingCallId: String,
        incomingCseq: String,
    ): String = "duplicate incoming INVITE final 200 OK callId=$incomingCallId cseq=$incomingCseq"
    ...
    ): String = "Sending $response via incomingResponseWriter=$hasIncomingResponseWriter"

    fun acceptedFinalResponseWriteContext(acceptedCallId: String): String = "incoming INVITE final 200 OK callId=$acceptedCallId"
    ...
    fun incomingAckAudioRouteSettleReason(): String = "incoming ACK audio route settle"
    ...
    fun rejectWithoutValidIncomingCallLog(call: Any?): String = "rejectCall without valid incoming currentCall: $call"
    ...
    fun localRejectResponse(callHeaders: Map<String, List<String>>): SipResponse {
        val responseHeaders =
            callHeaders - "rseq" - "require" - "content-type" - "p-access-network-info" +
